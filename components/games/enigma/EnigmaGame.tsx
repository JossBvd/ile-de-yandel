...
export function EnigmaGame({
  ...
  return (
    <>
      ...
      />
      {questionContainerVisible && (isMission2Step1 ? (
          <div
            className="absolute inset-0 z-10 flex items-center justify-center pointer-events-none"
            style={{
              padding: isSmallScreen ? "8px" : isMediumScreen ? "12px" : isDesktopSmall ? "16px" : "24px",
            }}
          >
            <div
            ...
            <div
              ...
              style={{
                ...
                boxShadow: "0 2px 12px rgba(0,0,0,0.12), 0 0 0 1px rgba(139,90,43,0.2)",
                padding: isMobileOrTablet ? (isSmallScreen ? "22px" : "26px") : isDesktopSmall ? "28px" : isDesktopMedium ? "34px" : "40px",
                maxHeight: "60dvh",
              }}
              role="region"
            ...
            >
              <div className="flex flex-row items-start gap-2 w-full">
                <div className="flex-1 min-w-0 flex flex-col items-center text-center" style={{ gap: isSmallScreen ? "10px" : "12px" }}>
                  {(() => {
                    const parts = game.text.split(/\n\n/).filter(Boolean);
                    const textSize = isMobileOrTablet ? (isSmallScreen ? "1.0625rem" : "1.125rem") : isDesktopSmall ? "1.25rem" : isDesktopMedium ? "1.375rem" : "1.5rem";
                    const sentenceSize = isMobileOrTablet ? (isSmallScreen ? "1.125rem" : "1.1875rem") : isDesktopSmall ? "1.25rem" : isDesktopMedium ? "1.375rem" : "1.5rem";
                    if (parts.length === 0) return <p className="text-gray-800 italic font-display w-full" style={{ fontSize: sentenceSize, lineHeight: 1.4 }}>« {game.text} »</p>;
                    return (
                      <>
                        <p className="text-gray-800 leading-tight italic font-display w-full" style={{ fontSize: textSize, lineHeight: 1.4 }}>{parts[0]}</p>
                        {parts.slice(1).map((paragraph, i) => (
                          <p key={i} className="text-gray-800 font-semibold leading-tight w-full whitespace-pre-line" style={{ fontSize: sentenceSize, lineHeight: 1.5 }}>{paragraph}</p>
                        ))}
                      </>
                    );
                  })()}
                </div>
                <ReadAloudButton text={game.text} ariaLabel="Lire l'énigme" className="shrink-0" />
              </div>
            </div>
            <div
              ...
              style={{
                backgroundImage: "url(/backgrounds/paper_texture.webp)",
                boxShadow: "0 2px 12px rgba(0,0,0,0.12), 0 0 0 1px rgba(139,90,43,0.2)",
                padding: isMobileOrTablet ? (isSmallScreen ? "12px 14px" : "14px 16px") : isDesktopSmall ? "14px 18px" : isDesktopMedium ? "18px 22px" : "20px 26px",
                gap: isSmallScreen ? "12px" : isMediumScreen ? "16px" : "22px",
              }}
            >
              <form
                onSubmit={handleSubmit}
                className="flex flex-row items-center flex-1 min-w-0"
                style={{ gap: isSmallScreen ? "16px" : isMediumScreen ? "20px" : "28px" }}
                aria-label="Formulaire de réponse à l'énigme"
              >
                <label htmlFor="enigma-answer-m2" className="sr-only">Votre réponse à l&apos;énigme</label>
                {hasError && <span id="enigma-error-m2" role="alert" className="sr-only">Réponse incorrecte. Saisissez une autre réponse.</span>}
                <input
                  id="enigma-answer-m2"
                  type="text"
                  value={answer}
                  ...
                  style={{
                    paddingLeft: isSmallScreen ? "14px" : isMediumScreen ? "16px" : "20px",
                    paddingRight: isSmallScreen ? "14px" : isMediumScreen ? "16px" : "20px",
                    paddingTop: isSmallScreen ? "12px" : "14px",
                    paddingBottom: isSmallScreen ? "12px" : "14px",
                    fontSize: isSmallScreen ? "0.9375rem" : isMediumScreen ? "1rem" : "1.125rem",
                ...
                >
                  <Image src="/ui/icon_bottle_send.webp" alt="" width={144} height={144} className="w-full h-full object-contain" />
                </button>
              </form>
            </div>
            </div>
